...
static int eth_post_probe(struct udevice *dev)
{
	...
	if (!is_zero_ethaddr(env_enetaddr)) {
		...
		if(0 == dev_seq(dev))
		{
			memcpy(pdata->enetaddr,(char *)sunxi_soc_chipid + 10,6);
			pdata->enetaddr[0] &= 0xFE;	//Ensure it is a unicast address
		}

		if(1 == dev_seq(dev))
		{
			memcpy(pdata->enetaddr,sunxi_soc_chipid + 2,6);
			pdata->enetaddr[0] &= 0xFE;	//Ensure it is a unicast address
		}

		printf("\n%s (eth%d) using static MAC address - %pM\n",
