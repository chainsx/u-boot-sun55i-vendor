...
int sunxi_md5_calc(u8 *dst_addr, u32 dst_len, u8 *src_addr, u32 src_len)
{
	...
	return _sunxi_md5_calc(dst_addr, dst_len, src_addr, src_len);
#else
...
{
#if !CONFIG_IS_ENABLED(CONFIG_DM)
	return _sunxi_sha_calc(dst_addr, dst_len, src_addr, src_len);
#else
...
{
#if !CONFIG_IS_ENABLED(CONFIG_DM)
	return _sunxi_rsa_calc(n_addr, n_len, e_addr, e_len, dst_addr, dst_len,
			       src_addr, src_len);
#else
					...
					struct key_prop *prop, uint8_t *out)
{
	...
	return sunxi_rsa_calc((u8 *)prop->modulus, prop->num_bits / 8, (u8 *)prop->public_exponent, prop->exp_len, out, 32, (u8 *)sig, sig_len);
}
			...
			void *obuf)
{
	...
	case HASH_ALGO_SHA256:
		return sunxi_sha_calc((u8 *)obuf, 32, (u8 *)ibuf, ilen);
	case HASH_ALGO_MD5:
		return sunxi_md5_calc((u8 *)obuf, 16, (u8 *)ibuf, ilen);
	default:
