...
static struct udevice *lt8912b_dev;
...
static struct sunxi_twi *lt8912b_twi;
...
static int i2c_dev_select(unsigned char chip_addr)
{
	struct udevice *bus;
	int ret;

	ret = uclass_get_device_by_seq(UCLASS_I2C, lt8912b_twi->bus_num, &bus);
	if (ret) {
		printf("Failed to get I2C bus\n");
		return ret;
	}

	ret = i2c_get_chip(bus, chip_addr, 1, &lt8912b_dev);
	if (ret) {
		printf("Failed to get I2C chip at 0x%02x\n", chip_addr);
		return ret;
	}
...
static int i2c_dev_write(unsigned char addr, unsigned char data)
{
	...
	ret = i2c_dev_select(I2CADR);

	if (ret){
		return ret;
	}

	ret = dm_i2c_write(lt8912b_dev, addr, &data, 1);
	return ret;
}
...
static int i2c_dev_read(unsigned char addr, unsigned char* data)
{
	...
	ret = dm_i2c_read(lt8912b_dev, addr, data, 1);
	return ret;
}
...
int lt8912b_probe(struct udevice *dev)
{
	lt8912b_twi = dev_get_priv(dev->parent);
	lt8912b_dev = dev;
...
static const struct udevice_id lt8912b_match[] = {
	{ .compatible = "panel-lt8912b-hdmi", },
};

U_BOOT_DRIVER(lt8912b) = {
	.name = "panel_lt8912b_hdmi",
	.id = UCLASS_I2C_GENERIC,
	.of_match = of_match_ptr(lt8912b_match),
	.probe = lt8912b_probe,
