...
struct dm_regulator_ops {
	...
	/**
	 * The most basic feature of the regulator dcdc output mode
	 *
	 * get/set_dcdc_mode - get/set enable state of the dcdc output mode
	 * @dev           - regulator device
	 * Sets:
	 * @enable         - set true - enable or false - disable
	 * @return true/false for get or -errno if fail; 0 / -errno for set.
	 */
	int (*set_dcdc_mode)(struct udevice *dev, int mode);
	int (*get_dcdc_mode)(struct udevice *dev);
};
...
/**
 * regulator_set_dcdc_mode: set regulator dcdc mode
 *
 * @dev    - pointer to the regulator device
 * @enable - set true or false
 * Return: - 0 on success or -errno val if fails
 */
int regulator_set_dcdc_mode(struct udevice *dev, int mode);

/**
 * regulator_get_dcdc_mode: get regulator dcdc mode
 *
 * @dev    - pointer to the regulator device
 * Return: - true/false of enable state or -errno val if fails
 */
int regulator_get_dcdc_mode(struct udevice *dev);
...
static inline int regulator_set_dcdc_mode(struct udevice *dev, bool enable)
{
	return -ENOSYS;
}

static inline int regulator_get_dcdc_mode(struct udevice *dev)
{
	return -ENOSYS;
}
#endif
