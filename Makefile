...
ifeq ("$(origin UARCH)", "command line")
  KBUILD_TOOL_ARCH:= $(UARCH)
endif
ifeq ("$(origin UA)", "command line")
  KBUILD_TOOL_ARCH:= $(UA)
endif
ifeq ("x$(KBUILD_TOOL_ARCH)", "xarm64")
CROSS_COMPILE ?= aarch64-linux-gnu-
else #arm and default
CROSS_COMPILE ?= arm-linux-gnueabi-
endif

DTS_PATH ?= $(PWD)/arch/arm/dts
#AW_DTS_PATH ?= $(srctree)/bsp/arch/arm/dts
export CROSS_COMPILE

$(shell echo $(CROSS_COMPILE) > $(srctree)/.crosscompile.txt)
...
BSP_UBOOTRELEASE = $(shell cat include/config/bsp_uboot.release 2> /dev/null)
UBOOTVERSION = $(VERSION)$(if $(PATCHLEVEL),.$(PATCHLEVEL)$(if $(SUBLEVEL),.$(SUBLEVEL)))$(EXTRAVERSION)
BSP_UBOOTVERSION = "bsp"
...
ifndef LDSCRIPT
	ifeq ($(wildcard $(LDSCRIPT)),)
		LDSCRIPT := $(srctree)/bsp/board/$(BOARDDIR)/u-boot.lds
	endif
	ifeq ($(wildcard $(LDSCRIPT)),)
		LDSCRIPT := $(srctree)/bsp/$(CPUDIR)/u-boot.lds
	endif
	ifeq ($(wildcard $(LDSCRIPT)),)
		LDSCRIPT := $(srctree)/bsp/arch/$(ARCH)/cpu/u-boot.lds
	endif
endif
...
UBOOTINCLUDE    := \
	-Ibsp/include \
	-Iinclude \
	$(if $(KBUILD_SRC), -I$(srctree)/bsp/include) \
	...
	$(if $(CONFIG_$(SPL_)SYS_THUMB_BUILD), \
		$(if $(CONFIG_HAS_THUMB2), \
			...
			-I$(srctree)/arch/arm/thumb1/include)) \
	-I$(srctree)/bsp/arch/$(ARCH)/include \
	-I$(srctree)/arch/$(ARCH)/include \
	-include $(srctree)/include/linux/kconfig.h \
	-I$(srctree)/bsp/include/openssl
...
HAVE_VENDOR_COMMON_LIB = $(if $(wildcard $(srctree)/bsp/board/$(VENDOR)/common/Makefile),y,n)

libs-$(CONFIG_API) += api/
libs-$(HAVE_VENDOR_COMMON_LIB) += bsp/board/$(VENDOR)/common/
libs-y += bsp/
libs-y += boot/
libs-y += cmd/
libs-y += common/
...
libs-y += $(if $(BOARDDIR),bsp/board/$(BOARDDIR)/)
...
INPUTS-$(CONFIG_ARCH_SUNXI) += u-boot-$(CONFIG_SYS_CONFIG_NAME).bin
...
#UBOOT_DIR = $(shell if [ -f $(LICHEE_BOARD_CONFIG_DIR)/uboot-$(VERSION)/uboot-board.dts ]; then echo uboot-$(VERSION); else echo ""; fi;)

#BOARD_DTS_EXIST = $(shell if [ -f $(DTS_PATH)/$(BOARD_DTS_NAME).dts ]; then echo yes; else echo no; fi;)

#ifeq ($(LICHEE_OUTPUT_CONFIGS), hdmi)
#DEVICE_BOARD_DTS_EXIST = $(shell if [ -f $(LICHEE_BOARD_CONFIG_DIR)/$(UBOOT_DIR)/uboot-board.dts ]; then echo yes; else echo no; fi;)
#else
#DEVICE_BOARD_DTS_EXIST = $(shell if [ -f $(LICHEE_BOARD_CONFIG_DIR)/$(UBOOT_DIR)/uboot-board-$(LICHEE_OUTPUT_CONFIGS).dts ]; then echo yes; else echo no; fi;)
#endif
...
#ifneq ($(LICHEE_OUTPUT_CONFIGS), hdmi)
#	@-cp -v $(LICHEE_BOARD_CONFIG_DIR)/$(UBOOT_DIR)/$(CONFIG_DEFAULT_DEVICE_TREE).dts $(DTS_PATH)/$(CONFIG_DEFAULT_DEVICE_TREE).dts
#endif
#	@-cp -v $(LICHEE_BOARD_CONFIG_DIR)/$(UBOOT_DIR)/uboot-board.dts $(DTS_PATH)/uboot-board.dts
#else
#ifeq (x$(BOARD_DTS_EXIST),xyes)
#	@-cp -v $(AW_DTS_PATH)/$(BOARD_DTS_NAME).dts $(DTS_PATH)/$(CONFIG_DEFAULT_DEVICE_TREE).dts
#else
#	@-cp -v $(AW_DTS_PATH)/$(CONFIG_SYS_CONFIG_NAME)-common-board.dts $(DTS_PATH)/$(CONFIG_DEFAULT_DEVICE_TREE).dts
#endif
#endif
...
TARGET_NODTB_BIN_NAME := u-boot-nodtb.fex
TARGET_DTB_BIN_NAME := u-boot-dtb.fex

u-boot-$(CONFIG_SYS_CONFIG_NAME).bin:   u-boot.bin
	@cp -v $<    $@
	@-cp -v $@ out/$(TARGET_BIN_NAME)
	@-cp -v $@ out/$(TARGET_BIN_NAME)
	@-cp -v u-boot-nodtb.bin out/$(TARGET_NODTB_BIN_NAME)
	@-cp -v u-boot-nodtb.bin out/$(TARGET_NODTB_BIN_NAME)
	@-cp -v u-boot.dtb out/$(TARGET_DTB_BIN_NAME)
	@-cp -v u-boot.dtb out/$(TARGET_DTB_BIN_NAME)

SUNXI_SCRIPT := $(srctree)/tools/pack/script
SUNXI_UPDATE_UBOOT := $(srctree)/tools/pack/update_uboot
SUNXI_DRAGONSECBOOT := $(srctree)/tools/pack/dragonsecboot

boot-package-sun55iw6: u-boot.bin
	$(info Prepare sunxi uboot files ...)
	$(shell rm -f u-boot.fex boot_package.fex boot_package.cfg sys_config.bin sys_config.fex monitor.fex scp.fex boot0_sdcard.fex optee.fex > /dev/null)
	$(shell cp u-boot.bin u-boot.fex)
	$(shell cp ./tools/pack/sun55iw6/sys_config.fex .)
	$(shell cp ./tools/pack/sun55iw6/boot0_sdcard.fex .)
	$(shell cp ./tools/pack/sun55iw6/sunxi.fex .)
	$(shell cp ./tools/pack/sun55iw6/monitor.fex .)
	$(shell cp ./tools/pack/sun55iw6/scp.fex .)
	$(shell cp ./tools/pack/sun55iw6/optee.fex .)
	$(shell cp ./tools/pack/boot_package.cfg .)
...
cmd_binman = $(srctree)/tools/binman/binman $(if $(BINMAN_DEBUG),-D) \
		...
		-I . -I $(srctree) -I $(srctree)/bsp/board/$(BOARDDIR) \
...
ENV_DIR := $(srctree)/bsp/board/$(BOARDDIR)
...
quiet_cmd_gen_envp = ENVP    $@
	...
	if [ -s "$(ENV_FILE)" ]; then \
		$(CPP) -P $(CFLAGS) -x assembler-with-cpp -D__ASSEMBLY__ \
			-D__UBOOT_CONFIG__ \
			-I . -I include -I $(srctree)/bsp/include -I $(srctree)/include \
...
endef
define filechk_bsp_uboot.release
	echo "$(BSP_UBOOTVERSION)$$($(CONFIG_SHELL) $(srctree)/scripts/setlocalversion $(srctree)/bsp)"
endef
...
# Store (new) UBOOTRELEASE string in include/config/bsp_uboot.release
include/config/bsp_uboot.release: include/config/auto.conf FORCE
	$(call filechk,bsp_uboot.release)
...
prepare3: include/config/uboot.release include/config/bsp_uboot.release
ifneq ($(KBUILD_SRC),)
...
define filechk_version.h
	...
	echo \#define BSP_PLAIN_VERSION \"$(BSP_UBOOTRELEASE)\"; \
	echo \#define U_BOOT_VERSION \"U-Boot \" PLAIN_VERSION; \
	echo \#define BSP_U_BOOT_VERSION \"U-Boot \" BSP_PLAIN_VERSION; \
...
$(version_h): include/config/uboot.release include/config/bsp_uboot.release FORCE
...
clean: $(clean-dirs)
	$(call cmd,rmdirs)
	$(call cmd,rmfiles)
	@find ./ bsp/ $(if $(KBUILD_EXTMOD), $(KBUILD_EXTMOD), .) $(RCS_FIND_IGNORE) \
